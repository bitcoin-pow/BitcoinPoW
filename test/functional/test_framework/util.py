...
BITCOIN_CONF_FILENAME = "bitcoin-pow.conf"
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, BITCOIN_CONF_FILENAME), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> tuple[dict, pathlib.Path]:
    ...
    if platform.system() == "Windows":
        ...
        datadir = temp_dir / "Bitcoin-PoW"
    else:
        env = dict(HOME=str(temp_dir))
        if platform.system() == "Darwin":
            datadir = temp_dir / "Library/Application Support/Bitcoin-PoW"
        else:
            datadir = temp_dir / ".bitcoin-pow"
    return env, datadir
...
def append_config(datadir, options):
    with open(os.path.join(datadir, BITCOIN_CONF_FILENAME), 'a') as f:
...
def get_auth_cookie(datadir, chain):
    ...
    if os.path.isfile(os.path.join(datadir, BITCOIN_CONF_FILENAME)):
        with open(os.path.join(datadir, BITCOIN_CONF_FILENAME), 'r') as f:
            for line in f:
